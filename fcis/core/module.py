...
from graphcut.superpixel_cache import super_p
    ...
    def init_params(self, initializer=Uniform(0.01), arg_params=None, aux_params=None,
        ...
        def _impl(name, arr, cache):
            ...
            if cache is not None:
                if name in cache:
                    if name == 'gt_masks_upsample_weight':
                        print 'here'
...
class MutableModule(BaseModule):
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        check_list = np.zeros(train_data.iters[0].size)

        save_path = 'data/test_network_warp'
        if not os.path.exists(save_path):
            os.mkdir(save_path)

        graph_cut_path = os.path.join(save_path, 'graph_cut_images')
        network_path = os.path.join(save_path, 'network_output_images')
        if not os.path.exists(graph_cut_path):
            os.mkdir(graph_cut_path)
        if not os.path.exists(network_path):
            os.mkdir(network_path)
        print 'created files to store graph cut masks and network output masks', graph_cut_path
        ...
        for epoch in range(begin_epoch, num_epoch):
            tic = time.time()
            eval_metric.reset()
            index = train_data.iters[0].index
            epoch_file = 'epoch_' + str(epoch)

            if not os.path.exists(os.path.join(graph_cut_path, epoch_file)):
                os.mkdir(os.path.join(graph_cut_path, epoch_file))
            if not os.path.exists(os.path.join(network_path, epoch_file)):
                os.mkdir(os.path.join(network_path, epoch_file))

            for nbatch, data_batch in enumerate(train_data):
                if monitor is not None:
                    monitor.tic()

                for idx, data in enumerate(data_batch.data):
                        data_batch.data[idx][-1] = nd.array([epoch])

                self.forward_backward(data_batch)
                self.update()
                self.update_metric(eval_metric, data_batch.label)

                seg_prob = self.get_outputs(0)[-5]
                gt_boxes = self.get_outputs(0)[-3]
                rois = self.get_outputs(0)[-4]
                gt_masks = self.get_outputs(0)[-1]
                ratio = self.get_outputs(0)[-6]

                for i in range(len(seg_prob)):
                    # get RGB image
                    im_tensor = data_batch.data[i][0].asnumpy()
                    im = np.zeros((im_tensor.shape[2], im_tensor.shape[3], 3))
                    for i_rgb in range(3):
                        im[:, :, 2 - i_rgb] = im_tensor[0, i_rgb, :, :] + 127

                    temp = []
                    for gt_idx, gt_box in enumerate(gt_boxes[i][0].asnumpy()):
                        # save graph_cut result
                        if index[nbatch * batch_size + i] <= train_data.iters[0].size / 2:
                            im_name = 'image_' + str(index[nbatch * batch_size + i]) + \
                                      '_box_' + str(gt_idx) + '_ratio_%.2f' % (ratio[i].asnumpy()[gt_idx]) + '.jpg'
                            gt_mask = gt_masks[i][gt_idx].asnumpy()
                            if gt_mask.shape != im[:,:,0].shape:
                                gt_mask = cv2.resize(gt_mask, im[:,:,0].T.shape)
                            masked_im = im * np.repeat(gt_mask[:, :, np.newaxis], 3, axis=2)
                            cv2.imwrite(os.path.join(graph_cut_path, epoch_file, im_name), masked_im)

                        for j in range(len(rois[i].asnumpy())):
                            if np.array_equal(rois[i].asnumpy()[j][1:], gt_box[:-1]):
                                temp.append(seg_prob[i][j].reshape((1,2,21,21)))

                                if index[nbatch * batch_size + i] <= train_data.iters[0].size / 2:
                                    # save network output image
                                    msk = seg_prob[i][j][1].asnumpy()
                                    ...
                                    if im[cod[1]:cod[3], cod[0]:cod[2], 0].size > 0:
                                        ...
                                        mskd_im[cod[1]:cod[3], cod[0]:cod[2], :] = mskd
                                        cv2.imwrite(os.path.join(network_path, epoch_file, im_name), mskd_im)
                                break
