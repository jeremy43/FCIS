...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, config, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.TRAIN.END2END:
            self.data_name = ['data', 'im_info', 'gt_boxes', 'color_hist', 'texture_hist',
                              'flow_avg', 'neighbor_matrix', 'segment', 'epoch']
        else:
            self.data_name = ['data']
    ...
    def parfetch(self, roidb):
        ...
        segment = load_cache(roidb[0]['segment'])
        # TODO: need to verify
        if roidb[0]['flipped']:
            segment['segment'] = segment['segment'][:, ::-1]
        ...
        data['color_hist'] = np.array([spixel.color_hist for spixel in segment['spixel']])
        data['texture_hist'] = np.array([spixel.texture_hist for spixel in segment['spixel']])
        data['flow_avg'] = np.array([spixel.flow_avg for spixel in segment['spixel']])
        # print data['flow_avg']
        data['neighbor_matrix'] = neighbor_matrix
