...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # max_data_shape.append(('flow', (config.TRAIN.BATCH_IMAGES,
    #           max([v[0] for v in config.SCALES]), max(v[1] for v in config.SCALES), 2)))
    print 'providing maximum shape', max_data_shape, max_label_shape

    # infer shape
    data_shape_dict = dict(train_data.provide_data_single + train_data.provide_label_single)
    data_shape_dict['color_hist'] = (350,75)
    data_shape_dict['texture_hist'] = (350, 60)
    data_shape_dict['flow_avg'] = (350, 2)
    data_shape_dict['neighbor_matrix'] = (350, 350)
    data_shape_dict['epoch'] = (1, )
    print 'data shape:'
