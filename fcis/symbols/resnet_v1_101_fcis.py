...
from operator_py.gt_annotator import *
from operator_py.box_parser import *
from operator_py.box_annotator_ohem import *
from operator_py.graph_cut import *
from operator_py.weighted_cross_entropy import *
...
class resnet_v1_101_fcis(Symbol):
    ...
    def get_seg_prob(self, conv_feat, relu1, cfg, im_info, gt_boxes, gt_masks=None,
                     gt_mask_weights=None, fcis_cls_seg=None, pre=False):
        ...
        if not pre:
            ...
            group = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape, gt_masks=gt_masks, gt_mask_weights=gt_mask_weights,
            ...
            gt_boxes_reshape = mx.sym.Reshape(data=gt_boxes, shape=(-1, 5), name='gt_boxes_reshape')
            rois, label = mx.sym.Custom(gt_boxes=gt_boxes_reshape, op_type='gt_annotator')
        ...
        if pre:
            ...
            return seg_pred, group, rpn_cls_score_reshape, rpn_bbox_pred
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            color_hist_list = mx.sym.Variable(name='color_hist')
            texture_hist_list = mx.sym.Variable(name='texture_hist')
            flow_avg_list = mx.sym.Variable(name='flow_avg')
            neighbor_matrix = mx.sym.Variable(name='neighbor_matrix')
            segment = mx.sym.Variable(name='segment')
            ...
            epoch = mx.sym.Variable(name='epoch')
        else:
            data = mx.sym.Variable(name="data")
            im_info = mx.sym.Variable(name="im_info")
            # gt_boxes = mx.sym.Variable(name="gt_boxes")
        ...
        if is_train:
            seg_pred = self.get_seg_prob(conv_feat, relu1, cfg, im_info, gt_boxes, fcis_cls_seg=fcis_cls_seg, pre=True)
            network_term = mx.sym.softmax(seg_pred, axis=1)
            gt_masks, gt_mask_weights, ratio = mx.sym.Custom(color_hist_list=color_hist_list,
                                                             texture_hist_list=texture_hist_list,
                                                             flow_avg_list=flow_avg_list,
                                                             neighbor_matrix=neighbor_matrix, segment=segment,
                                                             network_term=network_term,
                                                             op_type='GraphCut', gt_boxes=gt_boxes, data=data, epoch=epoch)

            seg_pred, group, rpn_cls_score_reshape, rpn_bbox_pred = self.get_seg_prob(conv_feat, relu1, cfg, im_info, gt_boxes, gt_masks, gt_mask_weights, fcis_cls_seg=fcis_cls_seg)
            ...
            # make roi gt
            # gt_boxes_reshape = mx.sym.Reshape(data=gt_boxes, shape=(-1, 5), name='gt_boxes_reshape')
            # rois, labels = mx.sym.Custom(gt_boxes=gt_boxes_reshape, op_type='gt_annotator', name='rois')
            ##################################
        ...
        if is_train:
            if cfg.TRAIN.ENABLE_OHEM:
                labels_ohem, mask_targets_ohem, bbox_weights_ohem, mask_weights_ohem = mx.sym.Custom(op_type='BoxAnnotatorOHEM', num_classes=num_classes,
                                                ...
                                                mask_targets=mask_reg_targets, bbox_targets=bbox_target, bbox_weights=bbox_weight, mask_weights=mask_weights)
                ...
                weighted_cross_entropy, softmax_out, label = mx.sym.Custom(op_type='weighted_cross_entropy',
                                                                    grad_scale=cfg.TRAIN.LOSS_WEIGHT[1] / cfg.TRAIN.BATCH_ROIS_OHEM,
                                                                    use_ignore=True, ignore_label=-1,
                                                                    seg_pred=seg_pred, mask_weights=mask_weights,
                                                                    label=mask_targets_ohem)
                seg_loss = mx.sym.MakeLoss(weighted_cross_entropy,
                                           normalization='null', name='seg_loss')
            ...
            group = mx.sym.Group(
                [rpn_cls_prob, rpn_bbox_loss, cls_prob, bbox_loss, seg_loss, mx.sym.BlockGrad(mask_reg_targets),
                 mx.sym.BlockGrad(rcnn_label), mx.sym.BlockGrad(ratio), mx.sym.BlockGrad(softmax_out),
                 mx.sym.BlockGrad(rois), gt_boxes,
                 im_info, gt_masks])
        else:
            cls_prob = mx.sym.SoftmaxActivation(name='cls_prob', data=cls_score)
            if cfg.TEST.ITER == 2:
            ...
            group = mx.sym.Group([rois, cls_prob, bbox_pred, seg_pred, im_info])
