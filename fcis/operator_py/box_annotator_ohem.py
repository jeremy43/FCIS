...
class BoxAnnotatorOHEMOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        mask_weights = in_data[7].asnumpy()
        ...
        mask_weights_ohem = mask_weights
        mask_weights_ohem[top_k_per_roi_loss[::-1][self._roi_per_img:],:,:,:] = 0
        ...
        mask_weights_ohem = mx.nd.array(mask_weights_ohem)

        for ind, val in enumerate([labels_ohem, mask_targets_ohem, bbox_weights_ohem, mask_weights_ohem]):
...
class BoxAnnotatorOHEMProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['cls_score', 'seg_pred', 'bbox_pred', 'labels', 'mask_targets', 'bbox_targets', 'bbox_weights', 'mask_weights']

    def list_outputs(self):
        return ['labels_ohem', 'mask_targets_ohem', 'bbox_weights_ohem', 'mask_weights_ohem']

    def infer_shape(self, in_shape):
        labels_shape = in_shape[3]
        mask_targets_shape = in_shape[4]
        bbox_weights_shape = in_shape[6]
        mask_weights_shape = in_shape[7]

        return in_shape, \
               [labels_shape, mask_targets_shape, bbox_weights_shape, mask_weights_shape]
