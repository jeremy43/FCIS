...
class ProposalAnnotatorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        gt_mask_weights = in_data[3].asnumpy()
        ...
        rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_weights = \
            self.sample_rois(all_rois, fg_rois_per_image, rois_per_image, self._num_classes,
                             self._cfg, gt_boxes=gt_boxes, gt_masks=gt_masks, gt_mask_weights=gt_mask_weights)

        for ind, val in enumerate([rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_weights]):
    ...
    def sample_rois(self, rois, fg_rois_per_image, rois_per_image, num_classes, cfg,
                    labels=None, overlaps=None, bbox_targets=None, gt_boxes=None, gt_masks=None, gt_mask_weights=None):
        if labels is None:
        ...
        mask_weights = np.zeros((len(keep_indexes), 2, self._mask_size, self._mask_size), )
        for idx, obj in enumerate(fg_indexes):
            ...
            mask_weights[idx] = gt_mask_weights[gt_assignment[obj]]
        ...
        mask_weights = mx.nd.array(mask_weights, dtype='float32', ctx=mx.cpu())

        return rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_weights
...
class ProposalAnnotatorProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['rois', 'gt_boxes', 'gt_masks', 'gt_mask_weights']

    def list_outputs(self):
        return ['rois_output', 'label', 'bbox_target', 'bbox_weight', 'mask_reg_targets', 'mask_weights']
    ...
    def infer_shape(self, in_shape):
        ...
        mask_weights_shape = (rois, 2, self._mask_size, self._mask_size)

        return in_shape, \
               [output_rois_shape, label_shape, bbox_target_shape, bbox_weight_shape, mask_reg_targets_shape, mask_weights_shape]
