...
class ImageNetVID(IMDB):
    ...
    def load_image_set_index(self):
        ...
        image_set_index_file = os.path.join(self.root_path, 'ILSVRC2015/ImageSets', self.image_set + '.txt')
        # print 'image set file', image_set_index_file
    ...
    def flow_path_from_index(self, index):
        ...
        # WARNING: last file does not exist but still returns its name
        # TODO: make sure it does not mess up
        ...
        flow_file = os.path.join(self.data_path, 'Flownet2', 'VID', list_index + '.flo')
    ...
    def segment_path_from_index(self, index):
        ...
        """
        if self.det_vid == 'DET':
            segment = os.path.join(self.data_path, 'cache', 'DET', index + '.pkl')
        else:
            segment = os.path.join(self.data_path, 'VID', index + 'warp.pkl')
    ...
    def load_vid_annotation(self, iindex):
        ...
        roi_rec.update({'boxes': boxes,
                        # 'flow': self.flow_path_from_index(index),
                        'segment': self.segment_path_from_index(index),
...
###################################################################################################
    def evaluate_detections(self, detections):
        ...
        self.write_vid_results_with_masks(detections)
        info = self.do_python_eval()
        return info

    def evaluate_sds(self, all_boxes, all_masks):
        info_str = self.evaluate_detections([all_boxes, all_masks])
        return info_str
    ...
    def write_vid_results_with_masks(self, detections):
        """
        write results files in pascal devkit path
        :param all_boxes: boxes to be processed [bbox, confidence]
        :return: None
        ...
        print 'Writing {} ImageNetVID results file'.format('all')
        filename = self.get_result_file_template().format('all')
        seg_filename = self.get_result_file_template().format('seg')
        seg_filename.replace('.txt', '.pkl')
        all_boxes = detections[0]
        all_masks = detections[1]
        with open(filename, 'wt') as f:
            for im_ind, index in enumerate(self.image_set_index):
                for cls_ind, cls in enumerate(self.classes):
                    if cls == '__background__':
            ...
            for im_ind, index in enumerate(self.image_set_index):
                for cls_ind, cls in enumerate(self.classes):
                    ...
                    for k in range(masks.shape[0]):
                        mask_cache.append(masks[k])
            cPickle.dump(mask_cache, f, protocol=cPickle.HIGHEST_PROTOCOL)
    ...
    def do_python_eval(self):
        ...
        imageset_file = os.path.join(self.root_path, 'ILSVRC2015/ImageSets', self.image_set + '_eval.txt')
        # print 'image_set', self.image_set
        annocache = os.path.join(self.cache_path, self.name + '_annotations.pkl')
        graphcutcache = os.path.join(self.data_path, 'cache', 'VID', '{0!s}.pkl')
        seg_crf_file = os.path.join(self.result_path, self.name + '_crf_masks.pkl')

        filename = self.get_result_file_template().format('all')
        seg_filename = self.get_result_file_template().format('seg')
        seg_filename.replace('.txt', '.pkl')
        ap, ap_seg, ap_gc = vid_eval(False, filename, seg_filename, seg_crf_file, graphcutcache, annopath, imageset_file, self.classes_map, annocache, ovthresh=0.5)
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
        ...
        # segmentation
        print '---------------------------------------'
        print 'Segmentation AP'
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('AP for {} = {:.4f}'.format(cls, ap_seg[cls_ind-1]))
            info_str += 'AP for {} = {:.4f}\n'.format(cls, ap_seg[cls_ind-1])
        print('Mean AP@0.5 = {:.4f}'.format(np.mean(ap_seg)))
        info_str += 'Mean AP@0.5 = {:.4f}\n\n'.format(np.mean(ap_seg))
        ...
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('AP for {} = {:.4f}'.format(cls, ap_gc[cls_ind-1]))
            info_str += 'AP for {} = {:.4f}\n'.format(cls, ap_gc[cls_ind-1])
        print('Mean AP@0.5 = {:.4f}'.format(np.mean(ap_gc)))
        info_str += 'Mean AP@0.5 = {:.4f}\n\n'.format(np.mean(ap_gc))
        return info_str
